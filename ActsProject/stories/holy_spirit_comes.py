from art import Terminal

__name__ = "the holy spirit comes"

def go():
    t = Terminal()

    speech_delay = 1.5

    wind_delay = 0.2

    fire_delay = 0.2

    language_delay = 1

    t.draw(
        """
              
        
                Pentecost
        
        
        """,
        1.5
    )

    t.draw(
        """
              

           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        1
    )

    t.draw(
        """
          Nothing interesting ever happens here!
         
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        speech_delay
    )

    t.draw(
        """  
              *  *      *     *****
            *      *  *  *   *     * *
             *****    ****    ******** 
                **      **      **
     Wind ->
     
        """,
        wind_delay
    )

    t.draw(
        """  
              *  *      *     *****
            *      *  *  *   *     * *
             *****    ****    ******** 
                **      **      **
     Wind ->     **      **      **
     
        """,
        wind_delay
    )

    t.draw(
        """  
              *  *      *     *****
            *      *  *  *   *     * *
             *****    ****    ******** 
                **      **      **
     Wind ->     **      **      **
                **      **      **
        """,
        wind_delay
    )

    t.draw(
        """  
              *  *      *     *****
            *      *  *  *   *     * *
             *****    ****    ******** 
                **      **      **
     Wind ->     **      **      **
                **      **      **
                 **      **      **
              """,
        wind_delay
    )

    t.draw(
        """
          ###    ##        ##     ##     ##
         ####      ##     ##      ##      ##
          ##      ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
         ###     ##        ##    ##     ##
          ####      ##    ##      ##      ##
          ##       ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
           ###   ##        ##    ##     ##
        ####       ##     ##      ##      ##
           ##     ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
             ###   ##        ##    ##     ##
           ####       ##     ##      ##      ##
         ##     ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
          ###    ##        ##     ##     ##
         ####      ##     ##      ##      ##
          ##      ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
         ###     ##        ##    ##     ##
          ####      ##    ##      ##      ##
          ##       ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
           ###   ##        ##    ##     ##
        ####       ##     ##      ##      ##
           ##     ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
             ###   ##        ##    ##     ##
           ####       ##     ##      ##      ##
         ##     ##       ##     ##     ##
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        fire_delay
    )

    t.draw(
        """
                Praise God!

           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        language_delay
    )

    t.draw(
        """
       Ylistaa Jumalaa !                  

           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        language_delay
    )

    t.draw(
        """
                           Lloat sigui Deu !            

           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        """,
        language_delay
    )

    t.draw(
        """
            The Holy Spirit has come upon us!
      
           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 

        """,
        speech_delay
    )

    t.draw(
        """        
           You were saying that nothing int-

           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 

        """,
        speech_delay - 0.5
    )

    t.draw(
        """        
               Shut up

           *       *       *      *      *
          ***     ***     ***    ***    ***  
           *       *       *      *      *
          * *     * *     * *    * *    * * 
        
          """,
        speech_delay
    )

    t.draw(
        """
                    Hey, these people speak our languages!

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay
    )

    t.draw(
        """
                           They must be drunk

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay
    )

    t.draw(
        """
                          Wait a minute

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay
    )

    t.draw(
        """
                         If they are drunk

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay
    )

    t.draw(
        """
                  then how does that let them speak in diff-

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay - 0.5
    )

    t.draw(
        """
                          You have any better ideas?

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay
    )

    t.draw(
        """
                                 Yes

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay
    )

    t.draw(
        """
                              Don't care

                            *     *     *
                           ***   ***   *** 
                            *     *     *
                           * *   * *   * *
        """,
        speech_delay
    )

    t.draw(
        """
          

            Peter Addressed the Crowd




        """,
        speech_delay
    )

    t.draw(
        """
            
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
                    Listen!
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
           These people are not drunk
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
               It's too early for that!
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
           
                      *                      
                     ***
                      *
                     * *
            ----------------------
   Not for me!          
        \   **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
                   ...okay
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
 But this is what the prophet Joel was talking about!
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
         God has poured out His Spirit
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
    Jesus was killed and raised from the dead
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
               He is the Messiah
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
(What should we do?)
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
(This seems pretty important)
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
(I'm hungry)
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay - 0.5
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
(Shut up)
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay - 0.5
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
(Seriously though, what should we do)
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
(Maybe we should ask him)
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
(No, he can't give any good advice!)
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
                I can hear you
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay - 0.5
    )

    t.draw(
        """
     
                      *                      
                     ***
                      *
                     * *
            ----------------------
So what should we do to be saved?
       \    **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
    Repent and be baptized for the forgiveness
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
                 of your sins
                      *                      
                     ***
                      *
                     * *
            ----------------------

            **********************
            **********************
           ************************ 
        """,
        speech_delay
    )

    t.draw(
        """
                                                     
                 The Game of Life                              
                              
                       
            Church Member Count:         
                                                 
                
                        
                                     
                                    
        """,
        speech_delay
    )

    t.draw(
        """
                                                     
                 The Game of Life                              
                              
                       
            Church Member Count: +3000          
                                                 
                
                        
                                     
                                    
        """,
        speech_delay
    )

    t.draw(
        """
                                                     
                 The Game of Life          
                              
                       
            Church Member Count: +3000     
                 
                                                 
            Your progress for today is: 
            
                                     
                                    
        """,
        speech_delay
    )

    t.draw(
        """
                                                     
                 The Game of Life                               
                              
                       
            Church Member Count: +3000

                                                 
            Your progress for today is: Excellent!
            
                                     
                                    
        """,
        speech_delay
    )

    t.finish(t.lineCount)